...
def _es_ordinal(token):
    ...
    sufijos = ("ra", "ro", "ta", "to", "er", "do", "va", "da")
...
# LIMPIEZA DE TITULO
# ============================================================
def _eliminar_programa_del_inicio(titulo_tokens, prog_tokens):

    if not prog_tokens:
        return titulo_tokens

    if len(titulo_tokens) < len(prog_tokens):
        return titulo_tokens

    if titulo_tokens[:len(prog_tokens)] == prog_tokens:
        return titulo_tokens[len(prog_tokens):]
...
def limpiar_titulo(
    ...
    tokens = _eliminar_programa_del_inicio(tokens, prog_tokens)
